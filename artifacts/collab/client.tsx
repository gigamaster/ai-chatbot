...
export const collabArtifact = new Artifact<"collab", Metadata>({
  ...
  onStreamPart: ({ streamPart, setArtifact }) => {
    ...
    // Note: These event types don't exist in CustomUIDataTypes, so we'll comment them out
    // for now to fix the build errors
    /*
    if (streamPart.type === "data-collab-join") {
      setArtifact((draftArtifact) => {
        const collaborators = [
          ...(draftArtifact.metadata?.collaborators || []),
        ];
    ...
    if (streamPart.type === "data-collab-leave") {
      setArtifact((draftArtifact) => {
        const collaborators = (
          draftArtifact.metadata?.collaborators || []
        ).filter((id) => id !== streamPart.data.userId);
        return {
          ...draftArtifact,
          metadata: {
            ...draftArtifact.metadata,
    ...
    },
    {
      ...
      onClick: async ({ content }) => {
        ...
        navigator.clipboard.writeText(
          `${window.location.origin}/collab/mock-link`
        );
      },
    },
    {
