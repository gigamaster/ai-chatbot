...
export function useChatOperations() {
  ...
  const saveChat = useCallback(
    async (data: { id: string; userId: string; title: string }) => {
      setLoading(true);
      setError(null);
      try {
  ...
  const getChat = useCallback(async (id: string) => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to get chat";
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
  ...
  const getChatsByUser = useCallback(
    async (params: {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to get chats";
        setError(errorMessage);
        throw err;
      } finally {
        setLoading(false);
  ...
  const deleteAllChatsByUser = useCallback(async (userId: string) => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to delete all chats";
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
  ...
  const saveMessages = useCallback(
    async (data: { chatId: string; messages: any[] }) => {
      setLoading(true);
      setError(null);
      try {
  ...
  const saveVote = useCallback(
    async (data: { chatId: string; messageId: string; isUpvoted: boolean }) => {
      setLoading(true);
      setError(null);
      try {
  ...
  const getVotes = useCallback(async (chatId: string) => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to get votes";
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
  ...
  const saveSuggestions = useCallback(async (data: { suggestions: any[] }) => {
    setLoading(true);
    setError(null);
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to save suggestions";
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
